package com.xb.common.entity.model.goods;

import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import lombok.Data;
import javax.validation.constraints.NotBlank;
import javax.validation.constraints.NotNull;
import java.io.Serializable;
import java.util.Date;

/**
 * <p>
 *
 * </p>
 *
 * @author xb
 * @since 2019-09-21
 */
@Data
@ApiModel(value="Product对象", description="")
public class Product implements Serializable {

    private static final long serialVersionUID = 1L;

    /**验证分组接口*/
    public interface Save{};
    public interface Update{};

    @NotNull(message = "id不能为空",groups = Update.class)
    @ApiModelProperty(value = "主键")
    private Integer id;

    @NotBlank(message = "商品名称不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "商品名称")
    private String name;

    @ApiModelProperty(value = "商品描述")
    private String description;

    @NotNull(message = "商品价格不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "商品价格")
    private Integer price;

    @NotNull(message = "商品库存不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "库存数量")
    private Integer stock;

    @NotNull(message = "商品分类1不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "分类1")
    private Integer level1Id;

    @NotNull(message = "商品分类2不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "分类2")
    private Integer level2Id;

    @NotNull(message = "商品分类3不能为空",groups = {Save.class,Update.class})
    @ApiModelProperty(value = "分类3")
    private Integer level3Id;

    @ApiModelProperty(value = "主图名称")
    private String mainImg;

    @ApiModelProperty(value = "副图地址")
    private String subImgs;

    @ApiModelProperty(value = "状态(1：在售 2：下架 3：已删除)")
    private Integer status;

    @ApiModelProperty(value = "创建时间")
    private Date createTime;

    @ApiModelProperty(value = "修改时间")
    private Date updateTime;


}
